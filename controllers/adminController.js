...
const createUser = async (req, res) => {
  try {
    ...
    if (!["asha", "leader", "admin", "public"].includes(String(role).toLowerCase())) return res.status(400).json({ message: "invalid role" });
    const exists = await prisma.user.findUnique({ where: { email } });
    if (exists) return res.status(409).json({ message: "email already exists" });
    const hashed = await bcrypt.hash(password, 10);
    const user = await prisma.user.create({ data: { email, password: hashed, role: String(role).toLowerCase(), name: name || null } });
...
const listWaterTests = async (req, res) => {
  try {
    if (ensureAdmin(req, res)) return;
    const tests = await prisma.waterTest.findMany({ orderBy: { createdAt: "desc" } });
